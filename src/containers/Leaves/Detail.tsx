...
import { LEAVES_PAGE_URL, ADMIN_LEAVES_PAGE_URL } from "@/config/routes";
...
import { getDate, LoadingPage } from "@/utils";
...
const Detail = ({ admin }: { admin?: boolean }) => {
      ...
      dispatch(
        alertModalOpen({
          ...
          header: !status
            ...
            ? "Approved"
            : "Denied",
          message: !status && isErrorWithData(approve.error)
            ? approve.error.data?.error || approve.error.data?.detail
              ? String(approve?.error?.data.error || approve.error.data?.detail)
  ...
  return loading ? (
    <LoadingPage />
  ) : leave.error && isErrorWithData(leave.error) ? (
    <Error
      statusCode={leave.error.status || 500}
      title={leave.error.data.error || leave.error.data.detail || "An unexpected error occurred"}
    />
  ) : (
    <Container
      heading={admin ? "Leave Information (Admin)" : "Leave Information"}
      icon={{ link: admin && leave.data?.user?.email !== data?.email ? ADMIN_LEAVES_PAGE_URL : LEAVES_PAGE_URL }}
      title={leave.data?.id}
      refresh={{
